...
def index(request):
    """
    Renders the homepage with featured products, including categories, drum kits, and cymbals.

    **Context**
    - ``categories``: A queryset containing all product categories.
    - ``featured_drum_kits``: A queryset of the top 4 highest-rated drum kits, sorted by rating.
    - ``featured_cymbals``: A queryset of the top 4 highest-rated cymbals, filtered by categories
